...
const Parameters = {
  ...
  CloudfrontSSLCertificateIdentifier: {
    Type: 'String',
    Description: 'SSL certificate for frontend HTTPS protocol (must be in us-east-1)'
  },
  LoadBalancerSSLCertificateIdentifier: {
    Type: 'String',
    Description: 'SSL certificate for backend HTTPS protocol (must be available to LoadBalancer)'
  },
...
const Resources = {
  ...
  TaskingManagerLoadBalancerHTTPSListener: {
    ...
    Properties: {
      Certificates: [ {
        CertificateArn: cf.arn('acm', cf.ref('LoadBalancerSSLCertificateIdentifier'))
      }],
      DefaultActions: [{
        Type: 'forward',
