...
import { useQueryParams, encodeQueryParams, StringParam } from 'use-query-params';
...
export const useTeamMembersStatsQueryParams = () => {
  ...
  switch (action.type) {
    ...
    case 'FETCH_SUCCESS':
      return {
        ...
        isLoading: false,
        isError: false,
        stats: action.payload.teamMembersStats,
      };
    case 'FETCH_FAILURE':
      return {
        ...state,
...
export const useTeamMembersStatsQueryAPI = (
  ...
  const [state, dispatch] = useReducer(dataFetchReducer, {
    isLoading: true,
    isError: false,
    stats: initialData.teamMembersStats,
    ...
    const fetchData = async () => {
      ...
      try {
        const result = await axios({
          url: `${API_URL}teams/${teamId}/statistics/`,
          method: 'GET',
          headers: headers,
  ...
  }, [throttledExternalQueryParamsState, forceUpdate, token, extraQuery, teamId]);
